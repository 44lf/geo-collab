...
3. **`X` 有值** → `SELECT` 活账号（`platform_user_id == X`、`is_deleted == False`、`merged_into IS NULL`，排除 B）：
   ...
   - **存在 canonical C**（重复命中）：
     ...
     2. **加成员**：B 的登录用户非 C 的 owner 且未在成员表 → 插 `account_members(C.id, B_user.id, "login_dedup")`。
     3. **处理 B 行（按条件合并 — 决策）**：
        - B **无**发布记录/任务绑定 → 软删 B（`is_deleted=True`，`platform_user_id` 保持 NULL）。
        - B **有**记录/任务 → 不软删，置 `B.merged_into = C.id`（从可用列表排除、待发布记录仍发完）。
     4. `resolved_account_id = C.id`；写审计日志。
...
- `platform_user_id` 为 NULL 且拿到 `X` → `SELECT` 其它活账号（`X`、`is_deleted==False`、`merged_into IS NULL`，排除自己）：
  ...
  - **有 canonical C** → 自动合并：自己 owner 加为 C 成员（`backfill_merge`）；按 §4-3-③同样的**条件合并**处理自己这行（无历史软删；有历史标 `merged_into=C`）；写审计。
...
| 账号 usage 状态 | `GET /api/accounts/usage`（§7） | `use` |
...
## 7. 排队 + 可见状态（需求 #4，口径收紧 — 评审点 10）

**串行化机制：零新增。** 共享账号 = 单 canonical 行 + 单 `state_path`/`profile_key`，成员的发布/重登天然落在同一把 `BrowserProfileLock`（按 profile_key）+ 同一把 per-account 锁（按 canonical id）+ 全局信号量上自动串行；登录与发布在同一 profile_key 互斥（`owner_kind` `login`/`publish`）。

**可见状态：从既有数据派生，口径明确：**
- **覆盖范围**：canonical 自身 `profile_key` 上的**发布占用**与**对 canonical 的显式重登/检测占用**。
- **不覆盖**：B「不知情登录同一物理账号」的**进行中** dedup 登录——此时 B 还在自己临时 `profile_key` 上，抽到 X 前系统不知道它属于 C，故不计入 C 的占用/排队。可选后续：抽到 X 后把该 session 映射回 canonical 再显示（§11）。
- **数据来源**：占用者＝该 profile_key 的 `BrowserProfileLock` 行（`owner_kind`/`owner_id`/`acquired_at`）；排队＝针对该账号、`status ∈ pending/waiting_*` 且带 `queue_reason` 的 `PublishRecord` + 排队登录会话，计数。
- **暴露**：`GET /api/accounts/usage`（批量）→ 每账号 `{ busy, held_by:{user,kind,since}, queue_depth }`；需小解析器把 `owner_id`(record/session id) 映射回操作用户。账号页可见时轮询。
- **前端**：账号行状态徽标（如「占用中 · 张三 · 30s / 排队 2」）。纯展示，不改串行逻辑。
...
- **排队状态**：`GET /api/accounts/usage` 在有 lock 行 + 排队 record 时返回正确 holder / queue_depth；进行中 dedup 登录不误计入 canonical。
- **迁移**：`0047` 建表 + 加列。
...
- **进行中 dedup 登录映射回 canonical 的排队展示**（§7 口径扩展）。
...
- `server/app/modules/accounts/service.py` — `user_can_use_account`/`user_can_manage_account`、`list_accounts(viewer_id)` 排除 `merged_into`、`delete_account` 清成员、成员管理 service、批量回填 service。
- `server/app/modules/accounts/router.py` — 列表可见性下沉、管理路径鉴权、成员管理端点、`GET /api/accounts/usage`、`POST /api/accounts/backfill-identity`。
...
- `web/src/features/accounts/*`、`web/src/api/accounts.ts`、`web/src/types.ts` — owner/成员标识、管理按钮 gating、成员管理 UI、排队徽标 + usage 轮询、「身份未知」徽标 + admin 批量回填入口。
- `server/tests/` — 见 §10。
