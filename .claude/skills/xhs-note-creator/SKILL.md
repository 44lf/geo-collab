...
2. **选精简提示词**：`list_prompt_templates(scope="generation", platform="xiaohongshu")`
   ...
   **首行 = 小标题**：每张卡的第一行会被渲染器**自动加粗**当作小标题——把游戏名 / TOP 名次
   （如 `🥈 TOP2 | 黑暗料理王`）写在第一行即可，不用自己打 `**`。
   **卡片高度固定、正文要精简**：卡片是固定画布（不会被撑高），正文排在上方、配图**自适应
   填充下方剩余空间**——正文写太多会把配图挤小甚至挤没。所以每张卡只放「标题 + 2~4 个要点
   （+推荐指数/一句玩家评论）」，别塞满整屏，给配图留出空间。
