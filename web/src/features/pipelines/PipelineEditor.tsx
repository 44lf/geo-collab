...
import { listQuestionPools, listQuestionTypes, syncQuestionPool } from "../../api/question-pools";
...
function deriveCheckedRecordIds(
  types: QuestionType[],
...
function deriveUnitMap(
  types: QuestionType[],
  config: Record<string, unknown>,
): Map<string, PickerUnit> {
  ...
  for (const t of types) {
    ...
    const record_ids =
      on.length === 0 || rids.length === 0 ? [] : on.length === rids.length ? null : on;
    map.set(sent, {
      question_type: sent,
      record_ids,
      allowed_prompt_template_ids: [],
...
function unitsToConfig(map: Map<string, PickerUnit>): Record<string, unknown> {
  ...
  for (const u of map.values()) {
    const included =
      u.record_ids === null || (Array.isArray(u.record_ids) && u.record_ids.length > 0);
  ...
  return (
    <>
      <div className="schemeFieldLabel">
        问题类型 · 共 {types.length}{" "}
        类（勾选问题=启用该类型；可各自配模板/数量，留空则用下游生成节点兜底）
      </div>
              ...
              <div className="schemeLineHead">
                  ...
                  {checkedCount > 0 &&
                    (unitHasTemplate(u) && unitHasCount(u) ? (
                      <span
                        ...
                        title="该类型已自带模板+文章数，下游生成节点将直接用它"
                      >
                        已接管
                      </span>
                    ) : (
                      <span
                        style={{
                          ...
                          padding: "1px 6px",
                          borderRadius: 4,
                          background: "var(--bg-2, #f1f1f4)",
                          color: "var(--fg-3)",
                        }}
                        title="该类型未配模板或文章数，缺的部分将回退到下游生成节点的兜底"
                      >
                        将用兜底
                      </span>
                    ))}
                ...
                <div className="schemeLineActions">
                  <span style={{ color: "var(--fg-3)" }}>
                    已选 {checkedCount} / {t.questions.length}
                  </span>
...
export function PipelineEditor({
  ...
  const onPublish = async () => {
    ...
    setHasDraft(false);
    onChanged();
    toast(`已发布 v${version_no}`, "success");
  };
  const onDiscard = async () => {
    if (!window.confirm("丢弃未发布改动？")) return;
    await discardDraft(pipelineId);
    await load();
    onChanged();
  };
  const onRun = async () => {
  ...
  const onSyncQuestionPool = async (poolId: number) => {
    ...
    let result: QuestionSyncResult;
    try {
      result = await syncQuestionPool(poolId);
    } catch (error) {
    ...
    const questionPoolRefreshFailure = "同步成功，但刷新问题源失败";
    try {
      ...
      toast(
        error instanceof Error
          ? `${questionPoolRefreshFailure}：${error.message}`
          : questionPoolRefreshFailure,
        "error",
      );
    } finally {
      setSyncingPoolId(null);
  ...
  const aiGenMask = useMemo<{ template: FieldCoverage; count: FieldCoverage }>(() => {
    ...
    const cover = (ok: (u: Record<string, unknown>) => boolean): FieldCoverage => {
      ...
      const state: FieldCoverage["state"] =
  ...
  return (
    <div className="peEditor">
      <div className="peToolbar">
        ...
        <button className="peBtnPrimary" onClick={onPublish}>
          发布
        </button>
        <button onClick={onDiscard} disabled={!hasDraft}>
          丢弃草稿
        </button>
      ...
      <div className="peAddBar">
        ...
        {nodeTypes
          .filter((t) => t.type !== "ai_generate")
          .map((t) => (
            <button key={t.type} className="peAddBtn" onClick={() => addNode(t.type)}>
              + {t.label}
            </button>
          ))}
      </div>
      ...
      <div className="peLayout">
        ...
        <div className="peNodeList">
          ...
          {nodes.map((n, i) => (
            <div key={i}>
              <div
                className={`peNode${i === selected ? " selected" : ""}`}
        ...
        <div className="pePanel">
          {sel && selDef ? (
            <div className="peCard">
              <div className="peCardTitle">
                {sel.name}
                <span className="peNodeType">{sel.node_type}</span>
              </div>
              ...
              {selDef.config_schema.map((f) => {
                ...
                if (
                  sel.node_type === "ai_compose" &&
                  (f.key === "web_search" || f.key === "deep_thinking")
                ) {
                  if (f.key === "deep_thinking") return null;
                  const dtField = selDef.config_schema.find((x) => x.key === "deep_thinking");
                  const webOn =
                    "web_search" in sel.config ? !!sel.config["web_search"] : !!f.default;
                  const dtOn =
                    "deep_thinking" in sel.config
                      ? !!sel.config["deep_thinking"]
                      : !!dtField?.default;
                  return (
                    <div className="agentField" key="model-capabilities">
                      ...
                      <div className="peCapabilityRow">
                        <div className="peCapabilityItem">
                          <span className="peCapabilityLabel">
                            <Globe size={15} />
                            联网搜索
                          </span>
                          <button
                            ...
                            onClick={() =>
                              updateNode(selected!, {
                                config: { ...sel.config, web_search: !webOn },
                              })
                            }
                          >
                        ...
                        <div className="peCapabilityItem">
                          ...
                          <button
                            ...
                            onClick={() =>
                              updateNode(selected!, {
                                config: { ...sel.config, deep_thinking: !dtOn },
                              })
                            }
                          >
                            <span className="peToggleKnob" />
                  ...
                  <label className="agentField" key={f.key}>
                    ...
                    {f.type === "question_pool" ? (
                      ...
                      >
                        ...
                        {engines.map((en) => (
                          <option key={en.model} value={en.model}>
                            {en.label || en.model}
                          </option>
                        ))}
                      </select>
              ...
              <div className="peSection">
                ...
                <label className="agentField">
                  ...
                  <select
                    value={sel.flow_meta?.dependsOnIndex ?? ""}
                    onChange={(e) =>
                      updateNode(selected!, {
                        flow_meta: {
                          ...(sel.flow_meta ?? {}),
                  ...
                  >
                    ...
                    {nodes
                      .filter((n) => n.node_index < sel.node_index)
                      .map((n) => (
                        <option key={n.node_index} value={n.node_index}>
                          #{n.node_index} {n.name}
                        </option>
                      ))}
                  </select>
                </label>
                <div className="agentFieldLabel" style={{ marginTop: 4 }}>
                  字段映射
                </div>
                  ...
                  <div className="peMapRow" key={mi}>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        updateNode(selected!, {
                          flow_meta: { ...(sel.flow_meta ?? {}), inputMapping: im },
                        });
                      }}
                    />
              ...
              <div className="peSection">
                ...
                <div className="peMapRow">
                  <input
                    type="text"
                    placeholder="字段"
                    value={sel.flow_meta?.condition?.field ?? ""}
                    onChange={(e) =>
                      updateNode(selected!, {
                        flow_meta: {
                          ...(sel.flow_meta ?? {}),
                  ...
                  />
                  <select
                    value={sel.flow_meta?.condition?.op ?? "eq"}
                    onChange={(e) =>
                      updateNode(selected!, {
                        flow_meta: {
                          ...(sel.flow_meta ?? {}),
                          condition: {
                            field: sel.flow_meta?.condition?.field ?? "",
