...
KNOWN_BUNDLE_SHAS: frozenset[str] = frozenset(
    {
        ...
        # v12 (2026-07-07, 合并两批改动):
        #  (a) 重试尊重锁定 + 重试失败通知(main): orchestrator SKILL 加「锁定与重试模型」——
        #      用户 `问题Id=` 精确锁问题词 / `生文提示词Id=` 锁模板时，评分不过按 worklist +
        #      「重写同一问题最多 REWRITE_CAP=2 次」而非随机换题；verifier 返回加 weak_dims +
        #      reasoning 喂给 writer 的重写模式做针对性改进；退出播报加 retry_exhausted 段
        #      单列「锁定问题重写到上限仍未过审」。同批 commands/goal.md 加锁定用法示例，
        #      writer SKILL 加「重写模式」段。本版顺带根治 service.build_bundle 跨 OS 排序
        #      （改 posix 串排序），bundle_sha 只剩行尾一个维度，每版只需登记 2 个 sha。
        #  (b) 飞书审核卡片 notify_review_card 发卡点(feishu): orchestrator SKILL 新增
        #      「可用工具」小节（含 notify_review_card）+ 主循环 verifier 决策后逐篇发卡
        #      （仅 approved / needs_rewrite，跳过 rejected）；收尾 notify_exit 批量汇总保留。
        # 下面两值由合并后的 SKILL.md 用 build_bundle_from_file_map 同款算法实算：
        # LF = CI canonical（git blob，模板全 LF），CRLF = autocrlf=true 本地工作区。
        "a2cf5ffd2ab5d0a2a846471c67d724cbe8bf4ccfc43dd44ae6d9d93149586b14",  # posix序+LF (CI canonical, blob, 全模板集合并重算)
        "f33cabd3d46b0c51513574b5c48ec0a382f79f479577ff3ccf5a37f4f18de01c",  # posix序+CRLF (autocrlf=true 本地工作区, 合并后重算)
    }
)
