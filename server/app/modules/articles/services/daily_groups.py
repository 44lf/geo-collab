...
sort_order、重复追加、commit 时机与并发行为是业务契约（详见
docs/superpowers/specs/2026-06-15-streaming-daily-group-design.md），本模块仅原样承接、不优化。
四个函数都用独立 session（session_factory）、本函数内 commit+close，尽力而为：失败记日志、不抛。
"""
