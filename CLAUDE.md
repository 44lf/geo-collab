...
- `mcp_catalog/` — MCP **只读 catalog 端点**（`router.py`，挂 `/api/mcp/*`，被 catalog 类 tool 复用）+ **接入指引**（`connect_router.py`）：`/api/mcp/status`（user JWT，回 `configured` / `suggested_base_url` / `tools_count`）+ `/api/mcp/health`（MCP token）。前端「MCP 接入」tab 用。**MCP 工具总数的唯一真值在 `connect_router.py:MCP_TOOLS_COUNT`（当前 36）**，增减 tool 时改这里。
...
POC 期：`server/mcp/` 跑独立 Python 进程（FastMCP stdio），把 GEO 现有 + 新增 API 包装成 ~36 个 atomic tools 给 Claude Code 调用（真值见 `mcp_catalog/connect_router.py:MCP_TOOLS_COUNT`）。Loop 配方在 `claude-loops/*.md`。
...
### Tool 三组（共 36 个，真值在 `mcp_catalog/connect_router.py:MCP_TOOLS_COUNT`）
