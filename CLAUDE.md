...
- `ai_generation/` — 问题池镜像飞书多维表（`question_bank.py`，`/api/generation/question-pools/*`；**问题池全员共享**，`QuestionPool.user_id` 只作创建者溯源，DELETE 仅 admin）。问题项可用性以 `source_active` 为准：默认 / `pending` 仅 active，`all` 全部，`consumed` 空列表，未知值 400；DTO 仍兼容返回 `status="pending"` 和 `article_id=null`。`/api/generation/ai-engines` / `format-engines` 仍供 Pipeline 与兼容客户端读模型候选。**站内唯一生文入口是 pipelines（UI 名称「智能体管理」）；MCP 是站外生文入口。**旧 `POST /api/generation/sessions` 已 410，历史 `GET /api/generation/sessions/{session_id}` 仍按用户私有规则可读。scheme 的 list/detail/history GET 只读保留；POST/PUT/PATCH/DELETE/run 全部 410 且零副作用，模块、历史表及 executor 仅保留给历史读取/内部兼容，不得重新接入。问题池同步线程与 Markdown 转换仍在本模块。
...
- `performance/` — 模板 / 账号产出 metrics 聚合：`GET /api/prompt-templates/{id}/performance` + `GET /api/accounts/{id}/performance` + `POST /api/publish-records/{id}/metrics`（回流写入，合并到 `Article.metrics` JSON 列）。模板表现按 `source_template_id` + 时间窗口真实聚合；缺指标为 `None`，非空窗口没有 approved 时 `approval_rate=0.0`。展示字段 `source_template_name` 不是聚合依据。
- `mcp_catalog/` — MCP **只读 catalog 端点**（`router.py`，挂 `/api/mcp/*`，被 catalog 类 tool 复用）+ **接入指引**（`connect_router.py`）：`/api/mcp/status`（user JWT，回 `configured` / `suggested_base_url` / `tools_count`）+ `/api/mcp/health`（MCP token）。前端「MCP 接入」tab 用。**MCP 工具总数的唯一真值在 `connect_router.py:MCP_TOOLS_COUNT`（当前 39）**，增减 tool 时改这里。
...
- `generation_sessions`、旧 LangGraph 源码、scheme executor 与 scheme 表均为历史休眠兼容资产：session 创建 POST 已 410，历史详情 GET 仍可读；scheme 只保留 list/detail/history GET，五个 scheme HTTP 写入口全为 410，不能启动 executor 或新写入。历史数据不删除、不重写。
