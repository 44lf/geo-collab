...
- `game_library/` — 游戏库语料底座：`games` / `game_tags` 存真实游戏与标签，复用 `stock_images`/`stock_categories` 存截图素材；`service.upsert_game()` 按 `name_normalized` 跨源并集合并，`scheduler.start_game_ingest()` 定时入库（开关 `GEO_GAME_INGEST_SCHEDULER_ENABLED`、周期 `GEO_GAME_INGEST_INTERVAL_SECONDS`、目标 `GEO_GAME_INGEST_TARGETS`），`server/scripts/ingest_games.py` 支持手动入库；MCP 只读检索端点 `/api/mcp/game-library/tags`、`/api/mcp/game-library/query` 对应 `list_game_tags` / `query_games_by_tags` 两个 tool。`save_article(... selected_games=...)` 会同事务回写游戏用量。
...
- `mcp_catalog/` — MCP **只读 catalog 端点**（`router.py`，挂 `/api/mcp/*`，被 catalog 类 tool 复用）+ **接入指引**（`connect_router.py`）：`/api/mcp/status`（user JWT，回 `configured` / `suggested_base_url` / `tools_count`）+ `/api/mcp/health`（MCP token）。前端「MCP 接入」tab 用。**MCP 工具总数的唯一真值在 `connect_router.py:MCP_TOOLS_COUNT`（当前 35）**，增减 tool 时改这里。
...
POC 期：`server/mcp/` 跑独立 Python 进程（FastMCP stdio），把 GEO 现有 + 新增 API 包装成 35 个 atomic tools 给 Claude Code 调用（真值见 `mcp_catalog/connect_router.py:MCP_TOOLS_COUNT`）。Loop 配方在 `claude-loops/*.md`。
...
### Tool 三组（共 35 个，真值在 `mcp_catalog/connect_router.py:MCP_TOOLS_COUNT`）
...
- **action**（写 15 个）：`save_article` / `illustrate_article` / `ai_illustrate_article` / `submit_review_decision` / `set_review_status` / `create_distribute_task` / `notify_feishu` / `report_event` / `install_loop_skills` / `compose_video` / `notify_review_card` / `record_adversarial_score` / `adopt_quality_reference` / `import_external_reference` / `get_external_reference_status`
...
- 生文跑在 API server 的后台线程，**没有独立 worker**。`create_app()` 把 `bg_session_factory = SessionLocal` 注入 `ai_generation.router` 和 `scheme_router`，并启动问题池定时同步线程（`start_auto_sync`）；如 `GEO_GAME_INGEST_SCHEDULER_ENABLED=true`，还会启动游戏库定时入库线程（`game_library.scheduler.start_game_ingest`），与 pipeline 调度同属 web 进程内后台线程。方案运行路由 spawn `Thread`，线程里 `scheme_executor` 用 `ThreadPoolExecutor(max_workers=4)` 并发跑 task，每个 worker 自建 session（session 非线程安全）。生产 `server/worker/executor.py` 不参与生文。
