...
- **action**（写 17 个）：`save_article` / `illustrate_article` / `ai_illustrate_article` / `submit_review_decision` / `set_review_status` / `create_distribute_task` / `notify_feishu` / `report_event` / `install_loop_skills` / `compose_video` / `notify_review_card` / `record_adversarial_score` / `adopt_quality_reference` / `import_external_reference` / `get_external_reference_status` / `compose_xhs_cards` / `get_xhs_status`
